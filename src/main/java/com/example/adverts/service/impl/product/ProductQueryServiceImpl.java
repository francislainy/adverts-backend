...
public class ProductQueryServiceImpl implements ProductQueryService {
    ...
    public ProductQueryDto getProduct(UUID id) {
        if (productRepository.findById(id).isPresent()) {
            ...
            return new ProductQueryDto(product.getId(), product.getTitle(), product.getDescription(), product.getPrice(), product.getCategory(), product.getSubCategory());
    ...
    public List<ProductQueryDto> getAllProducts() {
        ...
        productRepository.findAll().forEach(product -> {
            productList.add(new ProductQueryDto(product.getId(), product.getTitle(), product.getDescription(), product.getPrice(), product.getCategory(), product.getSubCategory()));
        });
    ...
    public List<ProductQueryNoParentDto> getAllProductsForCategoryAndSubCategory(UUID categoryId, UUID subCategoryId) {
        ...
        productRepository.findAll().forEach(product -> {
            ...
            if (product.getCategory().getId().equals(categoryId) && product.getSubCategory().getId().equals(subCategoryId)) {
                productList.add(new ProductQueryNoParentDto(product.getId(), product.getTitle(), product.getDescription(), product.getPrice()));
            }
